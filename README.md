...
本轮再新增 `HorizonConditioning`，把“h 是 p 的相关视域”与“h 作为条件规定 p 的可显现性”分开。Lean 已给出双向反模型：
...
所以“**不依赖视域**”“**不能被视域穷尽**”“**完全没有视域**”不能靠词面直接互换。

这些都是形式层答案，不自动等于对哲学家的解释。
...
但 **pp. 210–212、225–226 的原书正文仍未由本项目独立逐页核对**，所以项目尚未达到首个研究问题的停止条件，也不把 `Independent`、`HasExcess` 或 `HasSituatedExcess` 中任何一个冒称为 saturated phenomenon 的完整定义。

详见 [段落卡](docs/PASSAGE_CARDS.md) 与 [来源登记](docs/SOURCES.md)。

## 工程状态

Lean 4.24.0；不依赖 Mathlib 或其他外部 Lean 包。当前共有 **8 个库模块、46 个具名引理／定理**，全部列入内核公理依赖审计。
...
FormalPhenomenology/
  ...
  Models/Finite.lean
  Models/Conditioning.lean       # conditioning / excess / horizonless 分离反模型
...
首要硬门槛仍是**独立核对 Marion *Being Given* pp. 210–212、225–226 的原书正文**。conditioning 模型已经解决“该如何避免把不依赖视域偷换成无视域/不可穷尽”的工程问题；下一轮应继续突破原典访问门，再冻结最终解释归属，而不是继续无边界扩张模型。
